 ...
 * CORREÇÃO: Ajuste de Imports do Banco e Porta Real 3000
 *******************************************************************************/
...
const pg_1 = require("pg"); // <-- Instanciação nativa para evitar caminhos quebrados
...
const pool = new pg_1.Pool({ connectionString: process.env.DATABASE_URL || 'postgresql://recicla_user:recicla_password@127.0.0.1:5432/recicla' }); // <-- Inicializa o pool de conexões do PostgreSQL local
...
// Endpoint de retrocompatibilidade do KtorHttpClient (Bypass KMP)
...
/*******************************************************************************
 * 🔀 GET /api/v1/metrics/sankey
 * Unificado para corresponder estritamente ao fetch disparado pelo dashboard.html
 *******************************************************************************/
app.get('/api/v1/metrics/sankey', async (req, res) => {
    ...
    try {
      ...
      GROUP BY source
        ...
        console.error('Erro no endpoint /api/v1/metrics/sankey:', error);
        return res.status(500).json({ error: 'Erro ao consolidar regras horizontais do Sankey', details: error.stack || JSON.stringify(error) });
    }
});
/*******************************************************************************
 * BOOTSTRAP INITIALIZER - ALINHADO À PORTA REAL DO DOCKER COMPOSE
 *******************************************************************************/
const PORT = 3000;
app.listen(PORT, () => {
    console.log(`🚀 [Gateway API Recicla] Escutando e roteando na porta real ${PORT}`);
