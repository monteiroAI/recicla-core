"use strict";
/*******************************************************************************
 * ECOSSISTEMA DE RASTREABILIDADE LOGÍSTICA RECICLA (BACKEND CENTRAL)
 * CAMINHO FÍSICO: workspace/torre/recicla/src/modules/audit/controllers/forensicMissionController.ts
 * ============================================================================
 * RESPONSA: Tribunal Lógico Inviolável, Validação de Máquina de Estados e Gates Forenses.
 * DIRETRIZES:
 * 1. Mobile é canal burro de coleta leve. O Backend do Recicla processa o hardcore.
 * 2. Aplicação estrita do protocolo de 4 pesagens contra desvios/enxertos.
 * 3. Auditoria de telemetria (GPS tempo-espaço) e metadados EXIF de imagem.
 * 4. Handshake de encerramento temporizado (Contador regressivo local de 48 horas).
 *******************************************************************************/
Object.defineProperty(exports, "__esModule", { value: true });
exports.processarEstadoJornadaRecicla = void 0;
const postgres_1 = require("../../../shared/infra/postgres");
const schemaAtivo = "simulation";
// Parâmetros Físicos e Regulatórios de Tolerância Forense
const MARGEM_EVAPORACAO_MAXIMA = 0.02; // 2% de tolerância gravimétrica marginal em trânsito
const LIMITE_TEMPO_PARADO_SEGUNDOS = 900; // 15 minutos parado em local não autorizado desqualifica a missão
const RAIO_MAXIMO_GERADOR_METROS = 200; // Fotos do lote devem estar no raio de 200m da moega do cliente
const WINDOW_48H_RETENCAO_SEGUNDOS = 172800; // 48 horas exatas para expiração do cache no celular
/**
 * Função utilitária interna para cálculo de distância geográfica (Fórmula de Haversine)
 */
 ...
 */
const processarEstadoJornadaRecicla = async (req, res) => {
    ...
    try {
        ...
        if (proximo_estado === 'REJEITADA') {
            await client.query(`UPDATE recicla_missoes 
         ...
         WHERE uuid_missao = $2`, [dados_payload.motivo || "Não especificado", uuid_missao]);
        ...
        switch (proximo_estado) {
           ...
           WHERE uuid_missao = $1`, [uuid_missao]);
                break;
           ...
           WHERE uuid_missao = $2`, [dados_payload.peso_1, uuid_missao]);
                break;
            case 'T3_LOCAL_COLETA_CARGA': // Mínimo 2 fotos com carimbo espaço-tempo EXIF + Pesagem 2 + NF-e/MTR
                ...
                for (const foto of dados_payload.fotos_evidencia) {
                    ...
                    const distanciaDoGerador = calcularDistanciaGeografica(parseFloat(missao.cliente_latitude || 0), parseFloat(missao.cliente_longitude || 0), parseFloat(foto.latitude), parseFloat(foto.longitude));
                    if (distanciaDoGerador > RAIO_MAXIMO_GERADOR_METROS) {
                        await client.query(`UPDATE recicla_missoes SET status_missao = 'RETIDA_FRAUDE_VISUAL_LOCAL', elegivel_blockchain = false, status_auditoria = 'FOTO_FORA_DO_GERADOR' WHERE uuid_missao = $1`, [uuid_missao]);
                        await client.query('COMMIT');
                    ...
                    if (timeFoto > Date.now() || timeFoto < timeCriacaoOS) {
                        await client.query(`UPDATE recicla_missoes SET status_missao = 'RETIDA_FRAUDE_VISUAL_TEMPO', elegivel_blockchain = false, status_auditoria = 'TIMESTAMP_FOTO_INVALIDO' WHERE uuid_missao = $1`, [uuid_missao]);
                        await client.query('COMMIT');
                        return res.status(403).json({
                            success: false,
                    ...
                    await client.query(`INSERT INTO recicla_missoes_evidencias (id_missao, step, hash_sha256, foto_latitude, foto_longitude, foto_timestamp) 
             VALUES ($1, $2, $3, $4, $5, $6)`, [missao.id, foto.step, foto.hash_sha256, foto.latitude, foto.longitude, foto.timestamp]);
                }
           ...
           WHERE uuid_missao = $4`, [dados_payload.peso_2, dados_payload.mtr_numero, dados_payload.nfe_numero, uuid_missao]);
                break;
            ...
            case 'T4_GATE_CONSOLIDADO_CHECK': // Pesagem 3: Chegada na Cooperativa/Consolidadora (Auditoria contra Cargas Clandestinas)
                ...
                if (deltaMassa > limiteAdmissivel) {
                    await client.query(`UPDATE recicla_missoes SET status_missao = 'RETIDA_FRAUDE_MASSA', elegivel_blockchain = false, status_auditoria = 'DESQUALIFICADA_DELTA_PESO' WHERE uuid_missao = $1`, [uuid_missao]);
                    await client.query('COMMIT');
                    return res.status(403).json({
                        success: false,
                ...
                if (dados_payload.tempo_total_parado_segundos > LIMITE_TEMPO_PARADO_SEGUNDOS) {
                    await client.query(`UPDATE recicla_missoes SET status_missao = 'RETIDA_FRAUDE_TELEMETRIA', elegivel_blockchain = false, status_auditoria = 'DESQUALIFICADA_STOP_GPS' WHERE uuid_missao = $1`, [uuid_missao]);
                    // Incrementa índice relacional de conduta suspeita associado à ID estável do condutor
                    await client.query(`UPDATE equipe_motoristas SET ocorrencias_suspeitas = ocorrencias_suspeitas + 1 WHERE id_motorista = $1`, [id_motorista || missao.id_motorista]);
                    await client.query('COMMIT');
                    return res.status(403).json({
                        success: false,
                ...
                await client.query(`UPDATE recicla_missoes SET status_missao = 'T5_PROCEDIMENTO_DESCARGA', peso_3_bruto_destino = $1 WHERE uuid_missao = $2`, [peso3ChegadaDestino, uuid_missao]);
                break;
           ...
           WHERE uuid_missao = $2`, [peso4TaraDestino, uuid_missao]);
                await client.query('COMMIT');
...
exports.processarEstadoJornadaRecicla = processarEstadoJornadaRecicla;
