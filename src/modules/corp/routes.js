...
exports.setupCorpRoutes = void 0;
const express_1 = require("express");
const corpRouter = (0, express_1.Router)();
const setupCorpRoutes = (executarQuery) => {
    /**
     * POST /api/corp/os - Registrar Ordem de Serviço
     */
    corpRouter.post('/os', async (req, res) => {
     ...
     */
    corpRouter.post('/missoes/despachar', async (req, res) => {
        const { os_id, coordenador_id, motorista_id, equipamento_placa } = req.body;
        try {
            const resMissao = await executarQuery(`INSERT INTO corp_missoes (os_id, coordenador_id, status) VALUES ($1, $2, 'em-analise') RETURNING id;`, [os_id, coordenador_id]);
            const missaoId = resMissao.rows[0].id;
            const resViagem = await executarQuery(`INSERT INTO corp_missoes_viagens (missao_id, motorista_id, equipamento_placa, sequencia_viagem) VALUES ($1, $2, $3, 1) RETURNING *;`, [missaoId, motorista_id, equipamento_placa]);
     ...
     */
    corpRouter.post('/viagens/:id/evidencias', async (req, res) => {
        const { id } = req.params;
...
exports.setupCorpRoutes = setupCorpRoutes;
